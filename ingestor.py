...
def transform(rows, prodcat):
    ...
    return analytics, {"items": items, "gross": round(gross)}, \
           {k: round(x) for k, x in descdet.items()}, round(gross), py_data, daily
...
def main():
    ...
    A_all={};R_all={};D_all={};tot={};py_all={};daily_all={}
    all_ok = True
    for cliente, suc, branch, grupo in BRANCHES:
        ...
        A,R,D,g,py,daily = transform(rows, prodcat)
        A_all[branch]=A; R_all[branch]=R; D_all[branch]=D; tot[branch]=g; py_all[branch]=py; daily_all[branch]=daily
