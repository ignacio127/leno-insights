...
def rebuild_especial2026(master, P, args):
    # Leer template de productos del período anterior o del nivel raíz (legacy)
    esp_dict = master.get("especial2026", {})
    # Buscar template de productos: primero en el período previo, luego en raíz legacy
    template = None
...
def rebuild_mundiales(master, P, args):
    ...
    # Buscar template de productos del período previo
    template = None
...
def main():
    ...
    for cliente, suc, branch, grupo in BRANCHES:
        ...
        if abs(dq["gap_pct"]) <= IDENTITY_TOLERANCE_PCT:
            ...
            print(f"  🔒 {branch:14} CUARENTENA esta corrida — gap ${dq['gap']:,.0f} "
    ...
    if quarantined_now:
        ...
        print(f"  🔒 ATENCIÓN: {len(quarantined_now)} sucursal(es) puestas en CUARENTENA esta "
              f"corrida (dato fresco descartado, se conservó el último validado): {detalle_q}.")
    if cleared_now:
        print(f"  ✓ Salieron de cuarentena esta corrida: {', '.join(cleared_now)}.")
