...
def main():
    ...
    for cliente, suc, branch, grupo in BRANCHES:
        ...
        if not rows:
            ...
            if prev_A is not None and prev_tot is not None:
                ...
                print(f"  ⚠ {branch:14} SIN DATOS esta corrida — se CONSERVA el valor previo "
                      f"(${prev_tot:,.0f}) en vez de pisar con cero.")
                A_all[branch]=dict(prev_A); R_all[branch]=dict(prev_R) if prev_R else {"items":[],"gross":0}
