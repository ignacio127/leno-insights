    ...
    sc_list = sorted([
        {
            ...
            "items":   [{"nombre": i["nombre"], "u": int(i["u"]), "imp": round(i["imp"], 2)} for i in v["items"]],
        }
...
def rebuild_peya(master, P, py_by_branch, tot_by_branch, args):
    # Leer template de promos desde el master (formato plano viejo o por período)
    peya_src = master.get("peya", {})
    if "promos" in peya_src:
        promos_template = peya_src.get("promos", [])   # formato plano viejo
    else:
        any_p = next(iter(peya_src.values()), {}) if peya_src else {}
        promos_template = any_p.get("promos", [])
    ...
    for pr in promos_template:
        hit = rk_all.get(pr["nombre"],{})
        pr2 = {
            "nombre":   pr["nombre"],
            "cat":      pr.get("cat", "Always On"),
            "u":        hit.get("u", 0),
            "imp":      hit.get("imp", 0),
            "branches": dict(hit.get("branches", {})),
        }
    ...
    peya_new = {
        ...
        "diasBr": dias_br,
        "promos": promos_new,
        "cat":    {c:dict(v) for c,v in cat_acc.items()},
        "catBr":  {br:{c:dict(v) for c,v in d.items()} for br,d in cat_br_acc.items()},
        "cupones":{
    ...
    master.setdefault("peya", {})[P] = peya_new

# ===========================================================================
# REBUILD ESPECIAL 2026
# ===========================================================================
def rebuild_especial2026(master, P, args):
    # Leer el template de productos desde el master actual (puede estar en formato plano o por período)
    src = master.get("especial2026", {})
    # Obtener la lista de productos del template (independiente del período)
    if "productos" in src:
        productos_template = src["productos"]   # formato plano viejo
        srlBr = src.get("srlBr", [])
        frBr  = src.get("frBr", [])
    else:
        # ya está por período — tomar de cualquier período existente como template
        any_p = next(iter(src.values()), {}) if src else {}
        productos_template = any_p.get("productos", [])
        srlBr = any_p.get("srlBr", [])
        frBr  = any_p.get("frBr", [])
    ...
    productos = []
    for prod in productos_template:
    ...
    esp = {
        ...
        "productos": productos,
        "total": {
            "u":   sum(p["u"]   for p in productos),
            "imp": sum(p["imp"] for p in productos),
        },
    }
...
def rebuild_mundiales(master, P, args):
    # Leer template de productos desde el master (formato plano o por período)
    src = master.get("mundiales", {})
    if "productos" in src:
        productos_template = src["productos"]
        srlBr = src.get("srlBr", [])
        frBr  = src.get("frBr", [])
    else:
        any_p = next(iter(src.values()), {}) if src else {}
    ...
    for prod in productos_template:
        ...
        productos.append({
            ...
            "burger":   prod.get("burger", ""),
            "side":     prod.get("side", ""),
            "total":    {"u": hit.get("u", 0), "imp": hit.get("imp", 0)},
            "branches": dict(hit.get("branches", {})),
        })
    ...
    mund = {
        ...
        "frBr":      list(frBr),
        "productos": productos,
        "vasos":     sum(p["total"]["u"]   for p in productos),
        "totalImp":  sum(p["total"]["imp"] for p in productos),
    }
...
def main():
    ...
    for b in A_all:
        ...
        master["sc_comandas"][P][b] = list(sc_all.get(b, []))
