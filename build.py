...
function go(id){curSec=id;
 ...
 document.getElementById('secScope').textContent=NONPERIOD.indexOf(id)>=0?('Histórico de cadena · '+rangoHistoricoScope()+' · todas las sucursales'):(SELFSCOPE[id]?SELFSCOPE[id].scope:(DATA.period_meta[PERIOD].label+' · '+DATA.period_meta[PERIOD].scope));
 const chip=document.getElementById('periodChip');chip.textContent=NONPERIOD.indexOf(id)>=0?rangoHistoricoChip():(SELFSCOPE[id]?SELFSCOPE[id].chip:DATA.period_meta[PERIOD].label);
...
RENDER.fiscal=()=>{const brs=selBrs(fFisc);const c=mergeDict(brs,'comprobante');const lbl={'FAB':'FAB · Factura B','FAA':'FAA · Factura A','FAX':'FAX · Ticket/no fiscal','S/C':'Sin comprobante','NCB':'NCB · Nota de Crédito B','NCA':'NCA · Nota de Crédito A','NCX':'NCX · Nota de Crédito X'};
 ...
 const factor=grossFilt>0?1+descFilt/grossFilt:1;
 const arr=[...allComp].map(k=>({k:lbl[k]||k,v:Math.round((c[k]||0)*factor)})).sort((a,b)=>b.v-a.v);
 const tot=arr.reduce((s,x)=>s+x.v,0)||1;const el=document.getElementById('sec-fiscal');
 el.innerHTML=filterBar('fFisc','fiscal',fFisc)+'<div class="grid grid-cols-1 lg:grid-cols-2 gap-4"><div class="card p-5"><div class="font-semibold mb-3">Por tipo de comprobante</div><canvas id="cFisc" height="170"></canvas></div><div class="card p-5"><div class="font-semibold mb-3">Detalle</div>'+arr.map((x,i)=>'<div class="flex justify-between items-center py-3" style="border-bottom:1px solid var(--line)"><span class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full" style="background:'+PAL[i%PAL.length]+'"></span>'+x.k+'</span><div class="text-right"><div class="font-semibold">'+F(x.v)+'</div><div class="text-[12px]" style="color:var(--mut)">'+(x.v/tot*100).toFixed(1)+'%</div></div></div>').join('')+'</div></div>';
 ...
 // Drill-down S/C
 const scBrs=selBrs(fFisc);
 const scData=scBrs.flatMap(b=>{const cmd=((DATA.sc_comandas||{})[PERIOD]||{})[b]||[];return cmd.map(x=>({...x,suc:b}));}).sort((a,b)=>b.fecha.localeCompare(a.fecha));
 const scEl=document.getElementById('sec-fiscal');
 if(scData.length>0){
  const scTot=scData.reduce((s,x)=>s+x.importe,0);
  const showSuc=fFisc==='ALL';
  const cols=showSuc?6:5;
  let scHtml='<div class="card p-5 mt-4"><div class="flex items-center justify-between mb-3"><div class="font-semibold flex items-center gap-2"><span style="color:var(--warn)">⚠</span> Comandas Sin Comprobante</div><span class="badge" style="background:#fef3c7;color:#b45309">'+scData.length+' comanda'+(scData.length>1?'s':'')+' · '+F(scTot)+'</span></div><div class="overflow-x-auto"><table class="text-sm w-full"><thead><tr class="text-left text-[11px]" style="color:var(--mut)"><th class="py-2 pr-3">Fecha</th>'+(showSuc?'<th class="py-2 px-3">Sucursal</th>':'')+' <th class="py-2 px-3">Turno</th><th class="py-2 px-3">Canal</th><th class="py-2 px-3 text-right">Importe</th><th class="py-2 pl-2"></th></tr></thead><tbody>';
  scData.forEach((x,i)=>{
   const rid='scr'+i;
  ...
  scHtml+='</tbody></table></div>'+note('Comandas sin campo "ingreso" en Gesdatta. Puede ser error de sincronización del POS o cierre sin emisión de comprobante.')+'</div>';
