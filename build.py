...
RENDER.resumen=()=>{
 ...
 if(dqBrs.length){
   ...
   const avgPct=peor.reduce((s,x)=>s+Math.abs(x.gap_pct),0)/peor.length;
   const color=conFalla.length?'#e11d48':'#16a34a';
   const label=conFalla.length?conFalla.length+' sucursal(es) con desvío':'Todas las sucursales OK';
   dqCard='<div class="card p-4 mt-4" style="border-left:3px solid '+color+'">'+
     '<div class="flex items-center justify-between flex-wrap gap-2">'+
       ...
       '<span class="badge" style="background:'+(conFalla.length?'#fee2e2':'#dcfce7')+';color:'+color+'">'+label+'</span>'+
     '</div>'+
     '<div class="text-[12px] mt-1" style="color:var(--mut)">Desvío promedio: '+avgPct.toFixed(2)+'%'+
       (conFalla.length?' · '+conFalla.map(x=>x.b+' ('+x.gap_pct+'%)').join(' · '):'')+
     '</div></div>';
 }
