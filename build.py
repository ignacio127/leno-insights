...
const SELFSCOPE={burgermes:{chip:'Mar–May',scope:'Promo Burger del Mes · Mar–May 2026 · 7 sucursales'},mundiales:{chip:'Jun 08–14',scope:'Promo Nos Fuimos Mundiales · 08–14 Jun 2026 · 7 sucursales'},peya:{chip:'Jun 01–14',scope:'PedidosYa · 01–14 Jun 2026 · 7 sucursales'},especial2026:{chip:'Jun 01–14',scope:'Promo Especial 2026 · 01–14 Jun 2026 · en curso'}};
...
RENDER.peya=()=>{const PY=DATA.peya;const el=document.getElementById('sec-peya');const allBr=PY.srlBr.concat(PY.frBr);
 ...
 html+='</div>';
 html+=note('PedidosYa · 01–14 Jun 2026 · Barrio Sur y FLIP con datos parciales (mes en curso). Es un subconjunto del total de la red — no se suma aparte. Independiente del selector de período.');
 el.innerHTML=html;
...
RENDER.especial2026=()=>{const E=DATA.especial2026;const el=document.getElementById('sec-especial2026');const allBr=E.srlBr.concat(E.frBr);
 ...
 html+=note('Datos parciales del mes en curso (01–14 Jun). Los volúmenes son bajos porque la promoción recién inicia y el mes está incompleto — se completarán al cargar el período full o al conectar la API.');
 el.innerHTML=html;
};
...
RENDER.burgermes=()=>{const BM=DATA.burgermes,P=BM.promos;const el=document.getElementById('sec-burgermes');const allBr=BM.srlBr.concat(BM.frBr);
 ...
 html+='<div class="card p-5 mt-4"><div class="font-semibold mb-3">💡 Lectura ejecutiva</div><div class="grid md:grid-cols-2 gap-3 text-[13px]">'+
  ...
  '</div></div>'+note('Dataset propio de la promo “Burger del Mes” (Mar–May 2026, 7 sucursales, pre-agregado). Es independiente del selector de período del panel.');
 el.innerHTML=html;
...
RENDER.mundiales=()=>{const MD=DATA.mundiales,P=MD.productos;const el=document.getElementById('sec-mundiales');const allBr=MD.srlBr.concat(MD.frBr);
 ...
 '</div></div>'+note('Promo “Nos Fuimos Mundiales” (08–14 jun 2026, 7 sucursales, datos a nivel comanda). Solo Salón / Take Away — excluye LENO+ y PedidosYa. Independiente del selector de período del panel.');
 el.innerHTML=html;
};
...
twcss=open(_p('twbuild/tw.css'),encoding='utf-8').read()
