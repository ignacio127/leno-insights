...
function vdData(){
  ...
  return dd.map(r=>{
    let bruto=0;
    if(vdBr==='Total') bruto=r.Total||0;
    else if(vdBr==='SRL') bruto=r.SRL||0;
    else if(vdBr==='Franquicias') bruto=r.Franquicias||0;
    else bruto=r[vdBr]||0;
    return {fecha:r.fecha, v:Math.round(bruto*ratio)};
  });
}
RENDER.ventas_dia=()=>{
  ...
  el.innerHTML=
    ...
    +'</div>'
    +kpis
    +'<div class="card p-5 mb-4"><div class="flex items-center justify-between mb-3"><div class="font-semibold">Evolución diaria · '+(vdBr==='Total'?'Total cadena':vdBr)+'</div><span class="text-[11px]" style="color:var(--mut)">Ventas netas · desc. '+(100-ratio*100).toFixed(1)+'% proporcional del período</span></div>'
...
function vdPrint(){
  ...
  w.document.write('<html><head><title>Ventas Netas por Día · '+scope+' · '+pm.label+'</title>'
    ...
    +'<p class="note">Descuentos aplicados proporcionalmente ('+( 100-ratio*100).toFixed(1)+'% del período)</p>'
