...
const SELFSCOPE={
  ...
  mundiales:{chip:()=>{const md=(DATA.mundiales||{})[PERIOD]||(DATA.mundiales&&DATA.mundiales.rango?DATA.mundiales:null);return md?md.rango:'—';},scope:()=>{const md=(DATA.mundiales||{})[PERIOD]||(DATA.mundiales&&DATA.mundiales.rango?DATA.mundiales:null);return md?'Promo Nos Fuimos Mundiales · '+md.rango+' · 7 sucursales':'Sin datos para este período';}},
  peya:{chip:()=>{const py=(DATA.peya||{})[PERIOD]||(DATA.peya&&DATA.peya.rango?DATA.peya:null);return py?py.rango:'—';},scope:()=>{const py=(DATA.peya||{})[PERIOD]||(DATA.peya&&DATA.peya.rango?DATA.peya:null);return py?'PedidosYa Campañas · '+py.rango+' · 7 sucursales':'Sin datos para este período';}},
  especial2026:{chip:()=>{const e=(DATA.especial2026||{})[PERIOD]||(DATA.especial2026&&DATA.especial2026.rango?DATA.especial2026:null);return e?e.rango:'—';},scope:()=>{const e=(DATA.especial2026||{})[PERIOD]||(DATA.especial2026&&DATA.especial2026.rango?DATA.especial2026:null);return e?'Promo Especial 2026 · '+e.rango+' · en curso':'Sin datos para este período';}}
};
...
RENDER.resumen=()=>{
 ...
 if(pidx>0){const prev=DATA.periods[pidx-1];
   const agg=per=>{const A=DATA.analytics[per];let gr=0,d=0,c=0;SRL.forEach(b=>{if(A[b]){gr+=A[b].gross;d+=Math.abs(A[b].descuentos);c+=A[b].comandas;}});
   // Derivar días desde las fechas reales del label para evitar desincronismo con period_meta.dias
   // Si el label tiene formato "Mes YYYY (D1–D2)" extraer D2; si es mes completo usar dias del period_meta
   const pm=DATA.period_meta[per];let dd=pm.dias;
   const m=pm.label.match(/\((\d+)[–\-](\d+)\)/);if(m)dd=parseInt(m[2])-parseInt(m[1])+1;
   return{brutoDia:gr/dd,netoDia:(gr-d)/dd,ticket:gr/Math.max(c,1),descPct:d/gr*100};};
...
let pyBr='ALL';
RENDER.peya=()=>{const PY=(DATA.peya||{})[PERIOD]||(DATA.peya&&DATA.peya.rango?DATA.peya:null);const el=document.getElementById('sec-peya');
 ...
 html+='</div>';
 html+=note('PedidosYa · '+PY.rango+' · Es un subconjunto del total de la red — no se suma aparte. Independiente del selector de período.');
 el.innerHTML=html;
...
let e26Br='ALL';
RENDER.especial2026=()=>{const E=(DATA.especial2026||{})[PERIOD]||(DATA.especial2026&&DATA.especial2026.rango?DATA.especial2026:null);const el=document.getElementById('sec-especial2026');if(!E){el.innerHTML='<div class="card p-8 text-center" style="color:var(--mut)">Sin datos de campaña para '+PERIOD+'.</div>';return;}const allBr=E.srlBr.concat(E.frBr);
...
RENDER.mundiales=()=>{const MD=(DATA.mundiales||{})[PERIOD]||(DATA.mundiales&&DATA.mundiales.rango?DATA.mundiales:null);const el=document.getElementById('sec-mundiales');if(!MD){el.innerHTML='<div class="card p-8 text-center" style="color:var(--mut)">Sin datos de campaña para '+PERIOD+'.</div>';return;}const P=MD.productos;const allBr=MD.srlBr.concat(MD.frBr);
