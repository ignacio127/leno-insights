...
function vdData(){
  ...
  if(!dd.length) return [];
  const {SRL_,FR_}=VD_BRANCHES();
  return dd.map(r=>{
    if(vdBr==='Total') return {fecha:r.fecha, v:r.Total||0};
    if(vdBr==='SRL')   return {fecha:r.fecha, v:r.SRL||0};
    if(vdBr==='Franquicias') return {fecha:r.fecha, v:r.Franquicias||0};
    return {fecha:r.fecha, v:r[vdBr]||0};
  });
}
RENDER.ventas_dia=()=>{
  ...
  const tabla='<table class="text-sm w-full" id="vdTabla">'
    ...
    +'<th class="py-2 pr-4">Fecha</th><th class="py-2 px-3 text-right">Ventas</th>'
  ...
  el.innerHTML=
    ...
    +'</div>'
    +kpis
    +'<div class="card p-5 mb-4"><div class="font-semibold mb-3">Evolución diaria · '+(vdBr==='Total'?'Total cadena':vdBr)+'</div>'
...
function vdPrint(){
  ...
  w.document.write('<html><head><title>Ventas por Día · '+scope+' · '+pm.label+'</title>'
    +'<style>body{font-family:system-ui,sans-serif;padding:28px;color:#111}h2{margin:0 0 4px}p{margin:0 0 20px;color:#666;font-size:13px}'
    ...
    +'<h2>Ventas por Día · '+scope+'</h2><p>'+pm.label+' · '+pm.scope+'</p>'
    +'<table><thead><tr><th>Fecha</th><th class="r">Ventas</th><th class="r">vs promedio</th></tr></thead><tbody>'
    +rows.map(r=>{
